...
#'
#' Create a level 0 (country) or 1 (states) administrative map of the contiguous states of the USA with an inset of Alaska and/or Hawaii. The administrative level 1 map does not include major lakes.
#'
...
#'
#' @param scale Character string to specify the scale level of the map. Currently only supports: \emph{1:50m}.
#'
...
usa_compactor <- function(admin_level = 1,
    ...
    if(admin_level == 0 & scale == '1:50') {
        ...
        map <- 'ne_50m_admin_1_states_provinces_lakes.geojson'
        }
